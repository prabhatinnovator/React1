...
import Image3 from "../../assets/act.png";
    ...
    if (existinggroupNamesData) {
      ...
      return( existingNotes.map((note, index) => (
       (userIdClicked === note.id) ?(
        <div style={{display: "flex" , justifyContent: "space-between",marginBottom: "20px"}} key={index}>  
         <div className={StyleNotes.notes} style={{width: "490vw",contentWrap: "break-word"}}> {note.notes}
      ...
      {userIdClicked > 0 ? (
        <div className={StyleNotes.NotesGroupNotes}>
          <div className={StyleNotes.NotesGroupHeading}>
            &nbsp; &nbsp; &nbsp;<span className={StyleNotes.backButton} onClick={() => window.location.reload()}><img src={Image4}/> &nbsp;</span>
